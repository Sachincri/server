...
class OTPService {
  ...
  async createOTP(email: string, type: string, phone: string | null = null): Promise<{ otp: string; otpId: string }> {
  ...
  async verifyOTP(email: string, otp: string, type: string): Promise<boolean> {
    ...
    if (!isValid) {
      otpDoc.attempts += 1;
      await otpDoc.save();
      throw ApiError.badRequest(`Invalid OTP. ${3 - otpDoc.attempts} attempts remaining`);
    }
